...
class OthelloPlayer():
    ...
    def AI_MOVE(self, board):
        ...
        best_move = None
        depth = 1
        opponent = -self.current_symbol
        time_limit = 3  # segundos

        while True:
            try:
                _, mv = minimax(board, self.current_symbol, depth,
                                -math.inf, math.inf,
                                True, opponent,
                                start, time_limit)
                if mv is not None:
                ...
                break
        # fallback: si nunca encontró nada, usar aleatorio válido
        if best_move is None:
            from random_ia import AI_MOVE as rnd
            return rnd(board, self.current_symbol)
        return best_move
